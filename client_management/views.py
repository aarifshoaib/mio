    ...
    def get(self, request):
        ...
        if keyword is not None:
            clients = models.AddCompanyModel.objects.filter(Q(client__client_name__icontains=keyword) |
                        ...
                        Q(client__phone_number__icontains=keyword))
        else:
            clients = models.AddCompanyModel.objects.all().order_by('client__client_id')
            ...
            try:
                ...
                for _ in range(no_of_vac):
                    ...
                    create_order = models.CreateOrderModel(ref_id=ref_id, company=company, description=description,
                                        ...
                                        dropbox_url=dbx_url, tiny_url=short_url, require_date=require_date)
                    create_order.save()
                messages.success(request, f'New Order Created - {company.company_name}. error1 - {error1}. error2 - {error2}')
                response_data = {'status': 'success'}
            except Exception as e:
...
class VoiceChangerView(View):
    ...
    def post(self, request):
        ...
        try:
            d = self.process_audio(audiofile, 1, 1.6)
            messages.success(request, f'{d.read()}')
        except Exception as e:
            messages.error(request, f'Error: {e}')
        return redirect('/client-management/voice-changer-view')