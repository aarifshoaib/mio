...
def fetch_new_email(request, id):
    try:
        ec = models.EmailCredentialModel.objects.get(id=id)
        read_email_from_gmail(ec.gmail_id, ec.gmail_app_password, ec)
...
class ReadAllUnreadMailsView(View):
    def get(self, request):
        ...
        return redirect('/mom-application/unread-email')
    ...
    def get(self, request):
        ...
        if keyword:
            app_queues = models.CreateApplicationQueueModel.objects.filter(Q(company__company_name__icontains=keyword)|
                        ...
                        Q(assigned_by__icontains=keyword)|Q(staff__icontains=keyword)|Q(status__icontains=keyword))
        else:
            app_queues = models.CreateApplicationQueueModel.objects.all()
