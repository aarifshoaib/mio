...
from client_management.models import NewClientModel, AddCompanyModel, CPFTrackerCompanyModel
...
class YoursTaskView(View):
    ...
    def get(self, request):
        your_tasks = models.CreateTaskModel.objects.filter(asigned_to__user__username=request.user.username).order_by('-id')
    ...
    def get(self, request):
        ...
        if keyword:
            companies = AddCompanyModel.objects.filter(Q(company_name__icontains=keyword) |
                                                       ...
                                                       Q(company_review__date_of_incorp__icontains=keyword) |
                                                       Q(company_review__finance_year_end__icontains=keyword))
        else:
